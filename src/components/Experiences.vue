                    ...
                    <v-container>
                      <validation-observer ref="obs1">
                          ...
                          <v-col cols="8" offset="2">
                            ...
                            >
                              <v-text-field
                                ...
                                :label="$t('experiences_model_name')"
                        ...
                        <v-row>
                          <v-col cols="8" offset="2">
                            <validation-provider
                              name="Experience desciption"
                              rules="required"
                              v-slot="{ errors }"
                            >
                              <v-text-field
                                v-model="editedItem.description"
                                :label="$t('experiences_model_description')"
                                :error-messages="errors"
                              ></v-text-field>
                            </validation-provider>
                          </v-col>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      {
        text: i18n.t("experiences_model_id"),
        align: "start",
        sortable: false,
        value: "_id"
      },
      {
        text: i18n.t("experiences_model_name"),
        align: "start",
        sortable: false,
        value: "name"
      },
      {
        text: i18n.t("experiences_model_description"),
        align: "start",
        sortable: false,
        value: "description"
      },
  ...
  computed: {
    ...
    formTitle() {
      return this.editedId === -1
        ? i18n.t("experiences_add_new")
        : i18n.t("experiences_edit");
    }
  },
  ...
  methods: {
    ...mapActions({
      getPaginatedExperiences: "experience/getPaginated",
      storeExperience: "experience/store",
      updateExperience: "experience/update",
      deleteExperience: "experience/delete"
    }),
    ...mapMutations({
    ...
    deleteItem() {
      ...
      this.deleteExperience({ experienceId: this.selectedItemId })
        .then(response => {
          if (response) {
      ...
      this.$refs.obs1.validate().then(result => {
        if (result) {
          if (this.editedId !== -1) {
            ...
            this.setLoading(true);
            this.storeExperience({ data: this.editedItem })
              .then(response => {
                if (response) {
